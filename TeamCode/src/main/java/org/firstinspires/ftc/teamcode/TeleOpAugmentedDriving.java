 ...
 * targetA (x: -8, y: -22, heading: 90deg).
 * <p>
 * If B is pressed, the bot will generate a lineTo() trajectory on the fly and follow it to
 * targetB (x: 8, y: 22, heading: whatever the heading is when you press B).
 * <p>
 * If Y is pressed, the bot will turn to face 90 degrees, no matter its position on the field.
 * <p>
...
public class TeleOpAugmentedDriving extends LinearOpMode {
    ...
    Vector2d targetAVector = new Vector2d(0, -46);
    ...
    Vector2d targetBVector = new Vector2d(0, -31);
    ...
    public void runOpMode() throws InterruptedException {
        ...
        PoseStorage.currentPose = new Pose2d(0, -46, Math.toRadians(90));
            ...
            switch (currentMode) {
                case DRIVER_CONTROL:
                    ...
                    if (gamepad1.a) {
                        ...
                        currentMode = Mode.AUTOMATIC_CONTROL;
                    }else if (gamepad1.right_trigger > 0.75 && !trigger) {

                        vision.setCameraPipeline(OpenCVVision.Color.BLUE);
                            Trajectory runToSub = drive.trajectoryBuilder(poseEstimate)
                                    .lineTo(targetBVector)
                                    .build();
                           ...
                           yToBlock = -27.75 + (((vision.getTopRightCornerY() - 120) - 172.3076923076923) / 49.23076923076923);
                            ...
                            Trajectory runToBlock = drive.trajectoryBuilder(poseEstimate)
                                    .lineTo(targetBlockVector)
                                    .build();
                            drive.followTrajectoryAsync(runToBlock);
                    ...
                    }else if (gamepad1.left_trigger > 0.75 && !trigger) {
                        vision.setCameraPipeline(OpenCVVision.Color.RED);
                        Trajectory runToSub = drive.trajectoryBuilder(poseEstimate)
                                .lineTo(targetBVector)
                                .build();
